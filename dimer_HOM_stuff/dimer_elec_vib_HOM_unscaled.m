function [Time_units,rho_vec,nn,basis_proj]=dimer_elec_vib_HOM_unscaled(E1,V,omegavib,viblvls,...
...
%general up coupling
coup_p1 = zeros(size(nn,1));  %terms coupling upwards, no coefficient
coup_m1 = coup_p1;  %terms coupling downwards, with coefficient
coup_anticom = coup_p1; %terms coupling downwards with different operator

for k =1:sum(numwithn(1:end-1)) %top level doesn't couple upwards
    
    temp0 = repmat(nn(k,:),size(nn,1),1);
    temp = temp0 - nn;

    temp2 = sum(temp~=0,2) ==1; %only one element +/- 1 diff
    temp3 = sum(temp,2) < -1/2 & sum(temp,2) > -3/2; %ones with a - net about
       
    coup_p1(k,temp2 & temp3)=1;

end

for k =2:sum(numwithn(1:end)) %bottom level doesn't couple down
    ...
    coup_m1(k,temp2 & temp3)=diag(real(cc(temp5,temp6))).*diag(temp0(temp4,temp44));
    coup_anticom(k,temp2 & temp3)=diag(imag(cc(temp5,temp6))).*diag(temp0(temp4,temp44));
end

else
   nn=0;  coup_p1=0; coup_m1 =0; coup_anticom =0; %trivial case
end
...
%         choose_ele =  (kron(trueunit,c_anticom~=0))~=0;
        ...
        for j =1:N
           ...
           temp = temp + Qj{j}*(reshape(rho_vc(select_ele{k}),...
                    oneunit,sum(c_com~=0))*c_com(c_com~=0).');
