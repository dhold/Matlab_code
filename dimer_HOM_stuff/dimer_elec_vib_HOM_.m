...
% %assumed to be the same in each protein, choose (omega_1+omega_2)/2
 viblvls = 2; %Number of vibrational levels to include, integer
 coupg = 267.1; %coupling term between electronic and vibrational DOF
 tendps = 0.1; %end time in pico seconds
...
for k =1:sum(numwithn(1:end))
    nnsec = nn(k,1+(j-1)*(Kappa+1):j*(Kappa+1));
    temp0 = repmat(nnsec,size(nn,1),1);
    temp = temp0 - nn(:,1+(j-1)*(Kappa+1):j*(Kappa+1));
    ...
    coup_p1(k,temp2 & temp3)=dot(temp7,sqrt(1+nnsec(temp44)));
    end
    ...
    coup_m1(k,temp2 & temp3)= dot(temp7,sqrt(nnsec(temp44)));
    coup_m1_acom(k,temp2 & temp3) = dot(temp8,sqrt(nnsec(temp44)));
    end
    %elements with minus are the transpose of this 
    
end
coup_p1_save{j} = coup_p1;
coup_m1_save{j}= coup_m1; 
coup_m1_acom_save{j}= coup_m1_acom;
coup_p1 = 0*coup_p1; coup_m1 = coup_p1; coup_m1_acom=coup_p1;
end
