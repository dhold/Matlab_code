...
k3 = 2*pi*abs(om3)*kpr; %third beam to interact
k2 = 2*pi*abs(om2)*kpu; %first/second beam to interact
k1 = 2*pi*abs(om1)*kpu; %first/second beam to interact
%this approximates the wavevector by the average wavevector
%k_rp = -k1+k2+k3; k_nr = k1-k2+k3;  %outwavevector
...
S_rp = S_rp -1i*tmp*av_rp(:,ell,e3,e2,e1); 
S_rp_FO = S_rp_FO -1i*tmp*av_rp_FO(:,ell,e3,e2,e1); 
...
S_nr = S_nr -1i*tmp*av_nr(:,ell,e3,e2,e1); 
S_nr_FO = S_nr_FO -1i*tmp*av_nr_FO(:,ell,e3,e2,e1); 
...
S_rp_ft = sym([0;0]); S_nr_ft =sym([0;0]);
   ...
   S_rp_ft(1) = fourier(S_rp_ft(1)*heaviside(t3),t3,om);
