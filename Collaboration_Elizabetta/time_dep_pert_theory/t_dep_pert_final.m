...
if sys_label ==1 || sys_label ==2 %system 2 has the mode included in Hamiltonian 
...
Hvib = 1;
for j = 1:length(om_0)
coupg{j} = lambda{j}.^2./om_0{j};  Hvib_sec{j} = 1; Hvib_couple_sec{j} = 1;
if ~isempty(om_0{j})
...
%     end
    ...
    %assume there is only one mode per site for now
        ...
        Hvib_sec{j} = diag(0:viblvls{j})*om_0{j};
        
        temp = zeros(size(H0)); temp(j,j) = 1;
        tmp = kron(temp,diag(coupg{j}*sqrt(1:viblvls{j}),1));
...
H_vib = kron(Hvib_sec{1},eye(sum(szmat(2:end))));
Hvib_couple = kron(Hvib_couple_sec{1},eye(sum(szmat2(2:end))));
for j = 2:length(om_0)  
    H_vib = H_vib + kron(kron(eye(sum(szmat(1:j-1))),Hvib_sec{j})...
                        ,eye(sum(szmat(j+1:end))));    
    Hvib_couple = Hvib_couple + kron(kron(eye(sum(szmat2(1:j-1))),...
        Hvib_couple_sec{j}) ,eye(sum(szmat2(j+1:end))));    
end

H0full = kron(H0,eye(size(Hvib))) + kron(eye(size(H0)),H_vib) +...
        H_vib_couple ;
...
%% Plot first order terms
lambda_nm = 10^7./om_int_rng;
alpha_J_tp = matlabFunction(alpha_J,'vars',{om3});
%nav_tp = matlabFunction(nav,'vars',{om3,I});
figure
plot(lambda_nm,-alpha_J_tp(om_int_rng)/max(abs(alpha_J_tp(om_int_rng))))
%figure
%plot(om_int_rng,nav_tp(om_int_rng,sqrt(-1)))
xlabel('wavelength, nm')
ylabel('absorption coefficient \alpha, au')
eta_J_tp = matlabFunction(eta_J,'vars',{om3});

figure

plot(lambda_nm,-eta_J_tp(om_int_rng)/max(abs(alpha_J_tp(om_int_rng))))
xlabel('wavelength, nm')
ylabel('cd coefficient \eta, au')
%delta_J_tp  = matlabFunction(delta_J,'vars',{om3,I});
%figure

%plot(om_int_rng,delta_J_tp(om_int_rng,sqrt(-1)))
...
mu_op_L_ft{e1},fourier(mu_op_L{e1},t,0);
...
%       {(om_c+om_rel)/sqrt(2),(om_c-om_rel)/sqrt(2)}) ,16),[om,om_c,om_rel])
 ...
 %%
  ...
  %range 
  tstep = 0.01; tmax = 10; %change these values according to all the bath parameters
