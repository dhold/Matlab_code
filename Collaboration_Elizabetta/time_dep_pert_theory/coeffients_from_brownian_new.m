...
krng2 = find(lg2); 
krng3 = find(lg3);
 for klp = 1:length(krng1) %first underdamped most complicated due to the complex frequency
     k = krng1(klp);
  ...
  for klp = 1:length(krng2) %crit damped %not tested yet
         k = krng2(klp);     
  ...
  for klp = 1:length(krng3) %overrrrdamped, poles imaginary %not tested yet
     k = krng3(klp);
