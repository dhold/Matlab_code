...
%% Non wide pulse limit

%P1 ~ exp(-i*om_r*t)/2/pi * int(d om, exp(-i*om*t) S(om+om_r)*exp(-om^2*tau^2/2))
%P1 ~ exp(-i*om_r*t)/2/pi * int(d om, exp(-sqrt(2)*i*om*t/tau) S(sqrt(2)*om/tau+om_r)*exp(-om^2))
% Can evalute this with Gauss Hermite, or via FFT
%recall Env_r_ft = (pi^(-1/4)*exp(-(om3^2*tau_r^2))/2)*sqrt(abs(tau_r));
%omtilde = om * sqrt(2) /tau_r
% d om = d omtilde *tau_r/sqrt(2)
%Evaluate integral with Gauss Hermite quadrature
[omtilde, weights] = GaussHermite(9,eps(20));
omtilde = omtilde*tau_r/sqrt(2); %scale gaussian
weights = weights*pi^(-1/4)/sqrt(2*abs(tau_r));

final_trng_x = exp(linspace(log(tau_r/40),log(10*tau_r),55));
final_trng_y = linspace(tintrng(1),tintrng(end),200);
om_r_range = abs(E0) + linspace(H0ex(2,2)-200,H0ex(3,3)+200,100);

P1_non_imp_x = zeros(length(om_r_range),length(final_trng_x));
P1_non_imp_y = zeros(length(om_r_range),length(final_trng_y));
sym_to_fn('temp_fn_11.m',vpa(subs(S1ft(1),om3,om3+om_r),12),[om3,om_r])
sym_to_fn('temp_fn_22.m',vpa(subs(S1ft(2),om3,om3+om_r),12),[om3,om_r])

for lp =1:length(om_r_range)
    
    omr = om_r_range(lp);
tempx = temp_fn_11(omtilde,omr);   tempy = temp_fn_22(omtilde,omr);  

    for lp2 = 1:length(final_trng_x)
P1_non_imp_x(lp,lp2) = dot( tempx.*exp(-1i*omtilde*final_trng_x(lp2)), weights) ;
    end
    for lp2 = 1:length(final_trng_y)
P1_non_imp_y(lp,lp2) = dot( tempy.*exp(-1i*omtilde*final_trng_y(lp2)), weights) ;
    end    
...
P1x_fwd = zeros(length(om_r_range),numpoints);  %exp(i k.r)  term
P1y_fwd = zeros(length(om_r_range),numpoints);
...
for lp = 1:length(om_r_range)
    ...
    P1x_fwd(lp,:) = temp_fn_11(om_int_rng,omr); 
    P1y_fwd(lp,:) = temp_fn_22(om_int_rng,omr);
   ...
   P1x_fwd(lp,:) = ifft(intvar1); %now this is a function of time
   P1y_fwd(lp,:) = ifft(intvar2);

Sig_X_1(lp) = (4*pi*omr)*trapz(tintrng , imag( intvar0.*P1x_fwd(lp,:)));
Sig_Y_1(lp) = trapz(tint_ext,abs(P1y_fwd(lp,:)).^2);
