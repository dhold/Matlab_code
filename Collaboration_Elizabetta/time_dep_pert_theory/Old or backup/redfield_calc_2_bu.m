...
%             for d = 1:LL   
%                 if ~any([a,b,c,d]==1)
%                 %for lp = 1:N %site loop, each site can have diff bath
%                     Gam_RF(a,b,c,d,:)= Ctilde(d,c,:).*Q_j(a,b,:).*Q_j(c,d,:);
%                 %end
%                end
%             end
%         end
...
if 1==1 %more clever way, tested to give the same thing as before
    
  R_red = -(permute(Gam_RF,[3,2,4,1])+conj(permute(Gam_RF,[2,3,1,4])));  
  %this produces the same bit as the other code
  
%next consider the other two expressions with delta functions
%contract the tensor along 2nd and 3rd indicies, in diff perms
tmp = diagsum(Gam_RF,2,3); %diagsum(permute(Gam_RF,[1,2,4,3]),2,3); 
%diagsum(permute(Gam_RF,[2,1,3,4]),2,3);
 II = eye(LL); %rep of delta functions
% IIac = reshape(II,1,LL,1,LL);  IIdb = reshape(II,LL,1,LL,1);
...
else %simple but really slow method
R_red = zeros(LL,LL,LL,LL);
for a = 1:LL
    for b = 1:LL
        for c = 1:LL
            for d = 1:LL
...
R_red(a,b,c,d) = R_red(a,b,c,d) - (Gam_RF(d,b,a,c)+conj(Gam_RF(c,a,b,d)));               
                if b==d
R_red(a,b,c,d)  = R_red(a,b,c,d)  + diagsum(Gam_RF(a,:,:,c),2,3);
                end
                if a==c
                    %first one should be zero anyway
R_red(a,b,c,d)  = R_red(a,b,c,d)  + diagsum(conj(Gam_RF(b,:,:,d)),2,3);
                end
              %  end
            end
        end
