...
%clear V2 J2
%tic
%red_propop = V*expm(J*t)*V^(-1); %too slowwww
%toc
%tic
...
sym_to_fn('temp_fn_11.m',vpa(P1_ft(1),12),[om3,om_r])
sym_to_fn('temp_fn_22.m',vpa(P1_ft(2),12),[om3,om_r])

om_r_range = abs(E0) + linspace(H0ex(2,2)-200,H0ex(3,3)+200,100);
om_int_rng = abs(E0) + linspace(H0ex(2,2)-30/tau_r,H0ex(3,3)+30/tau_r,numpoints);
%om_int_rng = 0:(1/tau_r/100):abs(E0) + H0ex(3,3)+100/tau_r;

%tintrng = (0:(length(om_int_rng)-1))*(om_int_rng (end));

P1x_fwd = zeros(length(om_r_range),numpoints);  %exp(i k.r)  term
P1y_fwd = zeros(length(om_r_range),numpoints);
%P1x_bc = zeros(length(om_r_range),numpoints);   %exp(-i k.r) term
%P1y_bc = zeros(length(om_r_range),numpoints);

exp_fc =  exp(-(tintrng).^2/tau_r^2/2)/sqrt(sqrt(pi)*tau_r/2);
        %can select any frequency from this by selectively shifting it
        %along, but easier just to not to 
 intvar = conj(double(subs(Env_r_ft,om3,om_int_rng)));     
for lp = 1:length(om_r_range)
    
    omr = om_r_range(lp);
    intvar1 = temp_fn_11(om_int_rng,omr);  intvar2 = temp_fn_22(om_int_rng,omr);
  ...
  % P1x_fwd(lp,:) = ifft(intvar_ft1); %now this is a function of time
   %P1y_fwd(lp,:) = ifft(intvar_ft2);
   
   
Sig_X_1(lp) = (4*pi*omr)*trapz(om_int_rng, imag( intvar.*intvar1));
