...
%expm(supop*pi)-V*expm(J*pi)*V^(-1) = 0 or should
 ...
 V = PP(2); clear PP
%[V2,J2] = jordan(supop); %newer one reproduces original better
%sum(sum(abs(V2-V))) + sum(sum(abs(J2-J)))
%clear V2 J2
red_propop = V*expm(J*t)*V^(-1);
% [V,J] = eig(supop);
...
syms om_u om_r t_sep om1 om2 om3 reals  %om1..om3 are ft variables of t1..3
...
Env_r_ft = fourier(Env_r*(exp(1i*om_r*t3)+exp(-1i*om_r*t3)),t3,om3);
...
%express the exciton projection operators in the site basis for the HEOM
%code which uses the 
mu_unit{1} = ex_basis'*mu_unit{1}*ex_basis;
mu_unit{2} = ex_basis'*mu_unit{2}*ex_basis; 
for e1 = 1:2
mu_u_L{e1} = -kron(mu_unit{e1},eye(3)); %left acting commutator part
mu_u_R{e1} = kron(eye(3),mu_unit{e1});  %#ok<*SAGROW>%right acting, normal  
end
Rsp = norm(R12);
...
S1 = sym(zeros(3,1)); S1_nrwa = sym(zeros(3,1));
numpoints = 3000;  
...
S1_HEOM = zeros(length(tintrng),e1);
for e1 = 1:2
    ...
    S1 = S1 + temp;
    %kept in RWA
    %tempp = prefct(:,e1)*trace(reshape(mu_u_R_ex{e1}*red_propop1*mu_u_L_ex{e1}*rho0,3,3));     
    %S1_nrwa =  S1_nrwa + tempp;
    %lost in RWA

    clear temp temp2
    %Also use HEOM to test Redfield, not HEOM works in SITE BASIS
   [temp2,temp_trng,tmp2,tmp_t_2] = reduced_HEOM(...
            reshape(mu_u_R{e1}*rho0,3,3),...
             tintrng(end),H0,QQ,cc_com,cc_acom,vv,0,1,numpoints);
%this effectively factored out the global factor of exp(-i*|E0|*t), note by
%the convention we have taken E0 is negative
         
    %first 2 arguments will be zero for this one
if 1==0 %includes NRWA terms
    [temp2,temp_trng,tmp2,tmp_t_2] = reduced_HEOM(...
            reshape((mu_u_R{e1}+mu_u_L{e1})*rho0,3,3),...
             tintrng(end),H0,QQ,cc_com,cc_acom,vv,0,1,numpoints);
         
         rhoev_t = zeros(3,3,length(tintrng));
         rhoev_t(2,1,:) = exp(-1i*abs(E0)*tintrng).*interp1(temp_trng,temp2(:,1),tintrng);
        rhoev_t(3,1,:) = exp(-1i*abs(E0)*tintrng).*interp1(temp_trng,temp2(:,2),tintrng);
         rhoev_t(1,2,:) = exp(+1i*abs(E0)*tintrng).*interp1(tmp_t_2,tmp2(:,1),tintrng);
        rhoev_t(1,3,:) = exp(+1i*abs(E0)*tintrng).*interp1(tmp_t_2,tmp2(:,2),tintrng);
             rhoev_t = reshape( rhoev_t,9,length(tintrng));
             rhoev_t2 = mu_u_R{e1}*rhoev_t;
             rhoev_t2 = sum(rhoev_t2([1,5,9],:));
end
%     
%     for lpp = 1:length(temp_trng)
%        
%         test1 = zeros(3);  test1(1,2:end)= temp2(lpp,:);
%         test(lpp) = trace(reshape(mu_u_L{e1}*reshape(test1,9,1),3,3));
%         
%     end
    
    temp2 = mu_unit{e1}(1,2)*temp2(:,1) + mu_unit{e1}(1,3)*temp2(:,2); 
    %within linear spec in RWA this is 
        %just projected back to the ground state with unit amplitude
    %spline interpolate this over the intended trange
    
    S1_HEOM(:,e1)=  interp1(temp_trng,temp2,tintrng);
    %this bit is purely real and does not relate to the averaging of the
    %vector components of the dipole operator, which are included in prefct
    
    
    %set tsep to zero, you can just add this in later byshifting t
   NFFT = 2^nextpow2(2*numpoints-1); 
   S1_HEOM_FT(:,e1) = fft([flipud(S1_HEOM(2:end,e1));S1_HEOM(:,e1)],NFFT)/(2*numpoints-1);
    %0 to inf transform is simply half of this
    temp = fft(S1_HEOM(:,e1),NFFT)/(numpoints);
    figure
    plot(Fs/2*linspace(0,1/2,NFFT/2+1),2*real(temp(1:NFFT/2+1)))
     figure
     freq_rng = Fs/2*linspace(0,1,NFFT/2+1);

    plot(2*pi*freq_rng,abs(temp(1:NFFT/2+1))) %plot wrt angular freq in cm-1
end

S1_HEOM = 1i*S1_HEOM;   S1 = 1i*S1; %this factor also must be incluced
 
S1ft = fourier(S1*heaviside(t1),t1,om3);
om_int_rng = linspace(H0ex(2,2)-1000,H0ex(3,3)+1000,1000)+abs(E0);
int_arg = subs(Env_r_ft,t_sep,0).*S1ft;

P1 = ifourier(int_arg,om3,t);

%% first order polarization 
clear temp temp2 tmp2 tmp_t_2 P1x_imp P1y_imp P1xHEOM_imp P1yHEOM_imp

%P1 = int(exp(1i*om_r*t1)*S1,t1,[0,inf]); %can be done analytically but not by Matlab

temp_fn_11=matlabFunction(vpa(S1(1),12),'vars',[t1,om_r]);
temp_fn_22=matlabFunction(vpa(S1(2),12),'vars',[t1,om_r]);
om_r_range = abs(E0) + linspace(H0ex(2,2)-200,H0ex(3,3)+200,100);
%om_r_range = linspace(H0ex(2,2)-100,H0ex(2,2)+100,100);
 trng_x = linspace(-6*tau_r,8*tau_r,500); 
% %need much larger range as this signal is not heterodyned
% trng_y = linspace(-6*tau_r,1,1000); 
P1x = zeros(length(om_r_range),numpoints); 
P1y = zeros(length(om_r_range),numpoints);
P1xHEOM = P1x; P1yHEOM = P1y;
sigfct  = exp(-(trng_x).^2/tau_r^2/2)/sqrt(sqrt(pi)*tau_r/2);
%ft factor 

exp_fc =  exp(-(tint_ext).^2/tau_r^2/2)/sqrt(sqrt(pi)*tau_r/2);
...
for lp = 1:length(om_r_range)
    omr = om_r_range(lp);
    try
    prefactor = double(subs(prefct,om_r,omr));
    catch ME %tedious bug...
    prefactor = zeros(size(prefct));
    prefactor(1,:) = double(subs(prefct(1,:),om_r,omr)); 
    prefactor(2,:) = 1i*double(subs(imag(prefct(2,:)),om_r,omr));  
    end
    intvar1 = temp_fn_11(tintrng,omr);  intvar2 = temp_fn_22(tintrng,omr);
    intvrHEOM1 = prefactor(1,1).*S1_HEOM(:,1) + prefactor(1,2).*S1_HEOM(:,2);
    intvrHEOM2 = prefactor(2,1).*S1_HEOM(:,1) + prefactor(2,2).*S1_HEOM(:,2);
      
    % fourier transform to frequency domain
    S1_HEOM_FT_1 = fft([intvrHEOM1(end-1:2);intvrHEOM1])/(2*numpoints-1);
    S1_HEOM_FT_2 = fft([intvrHEOM2(end-1:2);intvrHEOM2])/(2*numpoints-1);
    intvar_ft1 = fft([intvar1(end-1:2);intvar1])/(2*numpoints-1);
    intvar_ft2 = fft([intvar1(end-1:2);intvar2])/(2*numpoints-1);  

    exp_fc_Ft = fft(exp_fc.*exp(1i*(omr)*tint_ext))/(2*numpoints-1);
    exp_fc_Ft_shift = fft(exp_fc.*exp(1i*(omr-abs(E0))*tint_ext))/(2*numpoints-1);
   ...
   P1x(lp,:) = ifft( exp_fc_Ft.*intvar_ft1); %now this is a function of time
   P1y(lp,:) = ifft( exp_fc_Ft.*intvar_ft2); 
   P1xHEOM(lp,:) = ifft( exp_fc_Ft_shift.*S1_HEOM_FT_1.');
   P1yHEOM(lp,:) =ifft( exp_fc_Ft_shift.*S1_HEOM_FT_2.');
%     for tlp = 1:length(trng_x)
%         tt = trng_x(tlp); 
% P1x(lp,tlp) = trapz(tintrng,exp(1i*omr*tintrng)...
%     .*exp(-(tt-tintrng).^2/tau_r^2/2)/sqrt(pi)/tau_r.*intvar1);
% P1y(lp,tlp) = trapz(tintrng,exp(1i*omr*tintrng)...
%     .*exp(-(tt-tintrng).^2/tau_r^2/2)/sqrt(pi)/tau_r.*intvar2);
% 
% P1xHEOM(lp,tlp) =trapz(tintrng,exp(1i*omr*tintrng)...
%     .*exp(-(tt-tintrng).^2/tau_r^2/2)/sqrt(pi)/tau_r.*intvrHEOM1.');
% P1yHEOM(lp,tlp) = trapz(tintrng,exp(1i*omr*tintrng)...
%     .*exp(-(tt-tintrng).^2/tau_r^2/2)/sqrt(pi)/tau_r.*intvrHEOM2.');
% 
%     end

    %delta fun limit
P1x_imp = exp(1i*omr*trng_x).*temp_fn_11(trng_x,omr);
P1x_imp(trng_x<0) = 0; %no polarization before pulse hits obviously
P1y_imp= exp(1i*omr*tintrng).*temp_fn_22(tintrng,omr);

P1xHEOM_imp= exp(1i*omr*trng_x).*interp1(tintrng,intvrHEOM1,trng_x);
P1xHEOM_imp(trng_x<=0) = 0; %no polarization before pulse hits obviously
P1yHEOM_imp = exp(1i*omr*tintrng).*intvrHEOM2.';    
    
Sig_X_1(lp) = (omr)*trapz(tint_ext, -imag(exp(1i*(omr)*tintrng).*exp_fc.*P1x(lp,:)));
Sig_Y_1(lp) = trapz(tint_ext,abs(P1y(lp,:)).^2);

Sig_X_1_HEOM(lp) = (omr)*trapz(tint_ext, -imag(exp(1i*(omr-abs(E0))*tintrng).*exp_fc.*P1xHEOM(lp,:)));
Sig_Y_1_HEOM(lp) = trapz(tint_ext,abs(P1yHEOM(lp,:)).^2);

Sig_X_imp(lp) = (omr)*trapz(trng_x, -imag(exp(1i*(omr-abs(E0))*trng_x).*sigfct.*P1x_imp));
Sig_Y_imp(lp) = trapz(tintrng ,abs(P1y_imp).^2);
Sig_X_imp_HEOM(lp) = (omr)*trapz(trng_x, -imag(exp(1i*(omr-abs(E0))*trng_x).*sigfct.*P1xHEOM_imp));
Sig_Y_imp_HEOM(lp) = trapz(tintrng ,abs(P1yHEOM_imp).^2);

CD_1(lp) = trapz(tint_ext,real(exp(1i*(omr)*tintrng).*P1y(lp,:).*exp_fc));
CD_1_HEOM(lp) = trapz(tint_ext,real(exp(1i*(omr-abs(E0))*tintrng).*P1yHEOM(lp,:).*exp_fc));
end
%% X signal
figure
...
hold on
plot(lambda_rng_nm,Sig_X_imp,'--')
plot(lambda_rng_nm,Sig_X_1_HEOM,'r')
plot(lambda_rng_nm,Sig_X_imp_HEOM,'--','color','r')
%% Y signal
figure
plot(lambda_rng_nm,Sig_Y_1)
hold on
plot(lambda_rng_nm,Sig_Y_imp,'--')
plot(lambda_rng_nm,Sig_Y_1_HEOM,'r')
plot(lambda_rng_nm,Sig_Y_imp_HEOM,'--','color','r')
 %P1 = P1*exp(-1i*om_r*t);
%% CD signal
...
plot(lambda_rng_nm,2*CD_1_HEOM,'r')
...
R1{e1,e2} = exp(+1i*om_u*t1)*red_propop1*mu_u_R{e1}*rho0;
R1{e1,e2} = red_propop2*mu_u_L{e2}*R1{e1,e2};
R1{e1,e2} = exp(1i*om_r*t3)*trace(reshape(mu_u_R{e1}*(red_propop3*mu_u_L{e2}*R1{e1,e2}),3,3));
...
R2{e1,e2} = exp(-1i*om_u*t1)*red_propop1*(mu_u_L{e2}*rho0);
R2{e1,e2} = red_propop2*(mu_u_R{e1}*R2{e1,e2});
R2{e1,e2} = exp(1i*om_r*t3)*trace(reshape(mu_u_R{e1}*(red_propop3*mu_u_L{e2}*R2{e1,e2}),3,3));
...
R3{e1,e2} = exp(-1i*om_u*t1)*red_propop1*(mu_u_L{e1}*rho0);
R3{e1,e2} = red_propop2*(mu_u_L{e1}*R3{e1,e2});
R3{e1,e2} = exp(1i*om_r*t3)*trace(reshape(mu_u_R{e2}*(red_propop3*(mu_u_R{e2}*R3{e1,e2})),3,3));
...
R4{e1,e2} = exp(+1i*om_u*t1)*red_propop1*(mu_u_R{e1}*rho0);
R4{e1,e2} = red_propop2*(mu_u_R{e1}*R4{e1,e2});
R4{e1,e2} = exp(1i*om_r*t3)*trace(reshape(mu_u_R{e2}*(red_propop3*(mu_u_R{e2}*R4{e1,e2})),3,3));
...
P3inte = sym(zeros(3,1));
for e1 = 1:2
    for e2  = 1:2
        P3inte =  P3inte  +  R1{e1,e2}*coeff1{e1,e2} + R2{e1,e2}*coeff2{e1,e2} ...
...
tic
sym_to_fn('temp_fn_1.m',vpa(P3inte(1)*Env_r*Env_u2*Env_u1,12),[t,t1,t2,t3,t_sep,om_r,om_u])
sym_to_fn('temp_fn_2.m',vpa(P3inte(2)*Env_r*Env_u2*Env_u1,12),[t,t1,t2,t3,t_sep,om_r,om_u])
toc
...
Int_Y_3 = trng_x*0; Int_Z_3 = trng_x*0;
IInt_Y_3 = trng_x*0; IInt_Z_3 = trng_x*0;
