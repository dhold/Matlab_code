function [Time_units,rho_vec,nn,total_prop_op]=multi_site_HEOM_plus_Efield...
            (Htot,QQ,rho_0,cc_com,cc_acom,vv,Kap1,Kap2,viblvls,Efield_at_t,...
            mu,numpoints,tendps,saveuptotier,use_reduced_mat,save_file_name)

%This code is used to model an N site system with the
%Heirachial orders method, stimulated by an electric field.
% Unit setup
...
%%
 % Htot is the system Hamiltonian.
 ...
 % cc_com = coefficients contributing to V^x (commutator)
 % cc2I = coefficients  contributing to V^o (anti commutator)
 % each of these is paired with an exponential with frequency vv
...
% Kappa %truncation parameter satisfying Kappa >>omega_0 beta hbar/2pi
 %beyond this e^(-v_(k>kappa) t)*v_k ~delta(t) is made, implicit
 ...
 % Kap2 e.g.= 4; %second truncation parameter this number is the maximum 
 % order the sum of the number of powers can take, 
 % i.e. n_1 + n_2 +... + n_Kappa <= Kap2
 % numpoints is the number of time points to save of the solved equation
 % can be given as a vector in order to save to disk as it runs
 % for all the auxilary density matricies
 % tendps e.g.= 2 is the end time in pico seconds
 % saveuptotier saves only up to this tier
 % use_reduced_mat only seems to work when no underdamped modes included
...
topass{1} = total_prop_op; 
    ...
    topass{2} = Efield_at_t{1};   %time range Efield calc'd for
    topass{3} = Efield_at_t{2};   %Efield at these times
...
tic %find better initial condition by finding lowest eigenvalue of prop operator
   ...
   if b < eps(10000) %b should be fairly small / zero for eq state
...
persistent total_prop Efld_rng trng mu
        ...
        if iscell(rho_vc)
            ...
            trng = rho_vc{2};
            Efld_rng = rho_vc{3};
            mu = rho_vc{4}; %transition dipole matrix elements
            drho =[];
