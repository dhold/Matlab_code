...
for j = 1:N    
    ...
    Qj{j} = -1i*(kron(eyeQ,Qjsec) - kron(Qjsec.',eyeQ)); %commtator
    %Qj{j} = U_tev*Qj*(U_tev'); %interaction picture
    %keep factor of -1i from -1i tilde(V)^X term
    Uj{j} =  (kron(eyeQ,Qjsec) + kron(Qjsec.',eyeQ));     
    %Uj{j} = U_tev*Uj*(U_tev'); %note U_tev is symbolic
end   
...
%     tmp_var = subs(U_tev ,tim,0);
% 
%     %identical constant factor 
%     
%     total_prop_op  = decay_term;
%     for j = 1 : N
%         
%         tmp_var2 = tmp_var' *Qj{j}* tmp_var;
%         
%     total_prop_op = total_prop_op + kron(sparse(coup_com_save{j}),tmp_var2);
%     total_prop_op = total_prop_op + kron(sparse(coup_acom_save{j}),tmp_var2);
%     
%     end
%     
%         tmp_var3 = tmp_var' *sparse(LL)* tmp_var;
%         tmp_var4 = tmp_var' *sparse(L2)* tmp_var;
%     
%         total_prop_op = total_prop_op + kron(spmat,tmp_var3);
%     prop_light =  kron(spmat,tmp_var4);

%% Propogate in time and solve system
topass{1} = {Qj,Uj,LL,spmat,decay_term, QQ,U_tev}; 
topass{2} = {coup_com_save,coup_acom_save,L2};  topass{3} = omega; 
...
persistent tim Qj Uj QQ LL L2 spmat decay_term U_tev omega ...
        coup_com_save coup_acom_save num_wvc rng rngex coup_plus coup_minus
        ...
        if iscell(rho_vc)
            ...
            LL = rho_vc{1}{3};
            spmat = rho_vc{1}{4};
            decay_term = rho_vc{1}{5};
            %trunc_op = rho_vc{1}{6};
            QQ = rho_vc{1}{6};
            U_tev = rho_vc{1}{7};

            L2 = rho_vc{2}{1};
            coup_com_save = rho_vc{2}{2};
            coup_acom_save = rho_vc{2}{2};
            
            omega = rho_vc{3};
            
            rngex(1) = 0; rng = 1:length(decay_term);
            ...
            for k =2:(2*num_wvc+1)
                rngex(k) = rngex(k-1) + length(decay_term);
            end
  ...
  tic
         tmp_var = subs(U_tev ,tim,t);
    
    total_prop_op  = decay_term; 
    for j = 1 : N
        
        tmp_var2 = tmp_var' *Qj{j}* tmp_var;
        tmp_var3 = tmp_var' *Uj{j}* tmp_var;
       trunc_opj = QQ(j,1).*tmp_var2*tmp_var2+ QQ(j,2).*tmp_var3*tmp_var2;      
    ...
    total_prop_op = total_prop_op + kron(sparse(coup_acom_save{j}),tmp_var3);
        ...
        total_prop_op = total_prop_op + kron(spmat,tmp_var' *sparse(LL)* tmp_var);
        total_prop_op = total_prop_op + kron(spmat,trunc_opj); %truncation
        % mixes density matricies to those with the same exponential phase
        % factor exp(il*k dot r)
        
    prop_light =  kron(spmat,tmp_var' *sparse(L2)* tmp_var);   
        %mixes to terms with l' = l +-1 
        
    pp_plus  = prop_light*exp(-1i*omega*t);
    pp_minus = prop_light*exp(+1i*omega*t);
        drho = zeros(rngex(end) + rng(end),1);
    toc
    tic
        for k = 1:num_wvc
            rng1 = rng+rngex(k);
            drho(rng1) = drho(rng1) + full(total_prop_op*rho_vc(rng1));
             %rngex(coup_plus(k,:))
