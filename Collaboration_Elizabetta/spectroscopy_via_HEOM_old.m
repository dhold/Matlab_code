function [OD,CD,SSden,chk,sup_op,sup_op_freq,selector] = spectroscopy_via_HEOM(Kap2 ,om_0,reorg_E,...
...
reorg_E = [0,25,25,25,25,25,25,25,25]; N = length(reorg_E)-1;
drude_gam = [0,35,35,35,35,35,35,35,35]; MM = -20;
om_0 = 12590;  Temp_kel = 77;
freq_range = linspace(om_0-100+25,om_0+100+25,20); %in wavenumber
dipole_vec = [ -0.823072771604003,	-0.5246284653639453,-0.21752284012024087;...
...
nn = zeros(sum(numwithn),tot_poles); 
count1=0; tmp3 = 0;
for k =2:Kap2+1
...
rho_eles = zeros((N+1)*size(nn,1),1);
rho_eles(1,1) =1; rho_eles(2:end,1) =eps(100)*rand((N+1)*size(nn,1)-1,1);  %init cond
if Kap2>0
...
clear down_coup_mat up_coup_mat
    ...
    temp = kron(const_fct,ones(N+1,1));
...
SSden = zeros(N+1,N,length(freq_range)); chk = zeros(N,length(freq_range));
...
for lp2 = 1:N   
      ...
      tmp = sparse(N+1,N+1); tmp(lp2+1,1) = -1; %#ok<SPRIX>
      selector = kron(padmat,tmp);
        opts.v0 = rho_eles;
   return
      [tmp1,tmp2] = eigs(-1i*(sup_op+sup_op_freq+selector),1,0,opts);
      
      chk(lp2,lp1) = tmp2(1,1); %should be near to zero, stationary state
      %occurs when eigenvalue is zero as this state doesn't change in time
      SSden(:,lp2,lp1) = tmp1(1:N+1,1);              
      
      OD(lp1) = OD(lp1) + ddotd(lp2,:)*tmp1(2:N+1,1);  
      %just imag of this relates to absorb but I might as well save it all
      CD(lp1) = CD(lp1) + CDop(lp2,:)*tmp1(2:N+1,1); 
end
end
OD = OD.*freq_range;  CD = CD.*freq_range; 
%dipole_vec should be 
%R_disp
