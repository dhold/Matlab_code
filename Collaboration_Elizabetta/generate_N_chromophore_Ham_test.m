...
om_0 = 1108;%[1108,1108,1108,1108]; %#ok<*NBRAK> %om_0 is the values of brownian modes included
lambda =44.32;%[44.32,44.32,44.32,44.32]; %reorganisation energy
gamma = 5.30884;%[5.30884,5.30884,5.30884,5.30884]; %damping of modes

viblvls = 0; %zero means no quantised vibration considered
%Could make this mode specfic, e.g. a vector
omegavib = [];%1108;%[938,1111,1
if ~isempty(omegavib)
sqm = lambda./omegavib;
end
lam_dru = 100;%[100,100,100,100]; %reorganisation energy of drude modes
gam_dru = 100;%[100,100,100,100]; %width
...
if viblvls > 0 && ~isempty(omegavib)
 ...
 %cc1 = zeros(N,Kappa); cc2R = cc1;  cc2I = cc2R; vv1 = cc1; vv2 = vv1;
 %QQ = zeros(N,1);
 for j=1:N
 [cc1,cc2R,cc2I,vv1,vv2,QQ,cc] = ...
...
cc1 = repmat(cc1,N,1); cc2R = repmat(cc2R,N,1); cc2I = repmat(cc2I,N,1); 
 ...
 numpoints =[120, [0.1:0.1:0.7]*convfact]; 
 saveuptotier =4;
 use_reduced_mat = false;
 vibstates = length(Hvib);
 %%  Run the full calculation
 
 [Time_units,rho_vec,nn2,total_prop_op2]=multi_site_drude_and_brownian_HOM_2...
            (Htot,QQ,rho_0,cc1,cc2R,cc2I,vv1,vv2,Kap1,Kap2,vibstates,...
            numpoints,tendps,saveuptotier,use_reduced_mat,'saved_data_test3.mat');
