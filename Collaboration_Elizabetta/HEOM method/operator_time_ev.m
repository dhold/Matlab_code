...
for j1 = 1:N
    ...
    for j2 = 1:N    
        ...
        sec_op = ex_basis(2,j2+1)*mu_ge_t{1}(1:f_leng,:)+...
            ex_basis(3,j2+1)*mu_ge_t{2}(1:f_leng,:);
...
%% Fit functions to mu_t in order to evaluate analytically easier
%multiple complex exponential fits would probably do

%%
error('just breaking here to avoid next bit')
...
for j1 = 1:N
    ...
    for j2 = 1:N 
        op2 = mu_ge_t{j2}(1:f_leng,:)+ mu_ef_t{j2}(1:f_leng,:);
        for j3 = 1:N
             op3 = mu_ge_t{j3}(1:f_leng,:)+ mu_ef_t{j3}(1:f_leng,:);
            for j4 = 1:N %this is the final interaction
                op4 = mu_ge_t{j4}(1:f_leng,:)+ mu_ef_t{j4}(1:f_leng,:);
