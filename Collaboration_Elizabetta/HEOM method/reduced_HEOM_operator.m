function [out1,t_range1,out2,t_range2,savehrarch] = reduced_HEOM_operator...
    (input_state,t_end, Htot,fock_space_rep,QQ,cc_com,cc_acom,vv,Kap1,Kap2,numpoints)
...
%not really important
    ...
    if test1 %only ground - 1st excited
        %
    Q1 = zeros(1,N); Q2 = zeros(1,N);
    for j = 1:N 
   
     %QQ(j,2) part acts as a commutator, QQ(j,1) part acts as an anti-com
     % Gives [0,set1;set2',zeros(N)] type (block) matrix

    Q1(j) = QQ(j,1) - QQ(j,2);
    Q2(j) = QQ(j,1) + QQ(j,2); 
    %second part is like a commutator (A V^x V^0 = [A ,V])and so
    %this picks up a negative element for the upper row which it acts on  
    %but as it is acting from the right this swaps

    end
    %add the truncation correction to the decay terms
    %Q1 = 0*Q1;  Q2 = Q2*0; %uncomment to test
...
% set1 = input_state(N+2:end,2:N+1); %first column
total_prop_op1 = -kron(spseye,sparse(-1i*Hsingle))+ decay_term1; 
%due to switching from density matrix to operator t dep it is now the row
%which has aquired a 
total_prop_op2 = kron(spseye,sparse(-1i*Hsingle))+ decay_term2;    
    
    elseif test2 %1st ex - double ex
        trunc_op_1 = zeros(length(Hsingle)*length(Hdouble)); 
        trunc_op_2  = trunc_op_1; 
    for j = 1:N 
    %test this part
    lg = fock_space_rep(:,j) & sum(double(fock_space_rep(:,j)),2)==2; 
    %states with excitation at j

    tmp1 = zeros(length(Hsingle),1);  tmp1(j) = 1;  
    tmp2 = zeros(length(Hdouble),1);  tmp2(lg) = 1;
    tmp1 = diag(tmp1); tmp2 = diag(tmp2);
    
    temp1 =kron(eye(length(Hdouble)),tmp1*tmp1);
    temp2 = kron(tmp2.' * tmp2.',eye(length(Hsingle)));
    ...
    temp1 = -1i*kron(sparse(eye(length(Hdouble))),sparse(Hsingle));
    temp2 = -1i*kron(sparse(Hdouble),sparse(eye(length(Hsingle))));
...
total_prop_op1 = kron(spseye,temp2-temp1)+ decay_term1;
%due to switching from density matrix to operator t dep it is now the row
%which has aquired a 
total_prop_op2 = kron(spseye,temp1-temp2)+ decay_term2;        
    end        
        ...
        for j = 1:N
            ...
            if test1
                %this was just done to illustrate to myself how the double
                %excited manifold logical is same is you replace Hdouble by
                %Hsingle and Hsingle by Hground, i.e. 0
%      tmp1 = zeros(length(0),1);  tmp1 = 0;  
%     tmp2 = zeros(length(Hsingle),1);  tmp2(j) = 1;
%     tmp1 = diag(tmp1); tmp2 = diag(tmp2);
%     
%     temp1 =kron(eye(length(Hsingle)),tmp1); %just zero
%     temp2 = kron(tmp2.',eye(length(0))); %just tmp2
    
    Qjsec = zeros(size(Hsingle));  Qjsec(j,j) = 1; 
    %Qjsec = kron(Qjsec,eye(viblvls));

    total_prop_op1 = total_prop_op1 ...
                    +1i*kron(sparse(coup_com_save{j}),Qjsec)...
                       + kron(sparse(coup_acom_save{j}),Qjsec) ;
    total_prop_op2 = total_prop_op2 ...
                        -1i*kron(sparse(coup_com_save{j}),Qjsec)...
                       + kron(sparse(coup_acom_save{j}),Qjsec) ;
            elseif test2
   ...
   %states with excitation at j           
    ...
    temp1 =kron(eye(length(Hdouble)),tmp1);
    temp2 = kron(tmp2.',eye(length(Hsingle)));
    %Qjsec = kron(Qjsec,eye(viblvls));
    total_prop_op1 = total_prop_op1 ...
                    -1i*kron(sparse(coup_com_save{j}),temp1-temp2)...
                       + kron(sparse(coup_acom_save{j}),temp1+temp2) ;
    total_prop_op2 = total_prop_op2 ...
                        -1i*kron(sparse(coup_com_save{j}),temp2-temp1)...
                       + kron(sparse(coup_acom_save{j}),temp1+temp2) ;
...
%pass this back so this section can be skipped for further calculations
    ...
    op_vec = zeros(numel(set1)*size(nn,1),1);
        ...
        outputfun(numpoints,op_vec,'notsavingnayway'); 
