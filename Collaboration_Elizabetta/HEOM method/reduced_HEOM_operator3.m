function [out1,t_range1,out2,t_range2,savehrarch] = reduced_HEOM_operator3...
    (input_state,t_end, Htot,fock_space_rep,QQ,cc_com,cc_acom,vv,Kap1,Kap2,numpoints,save_to_tier)
% UNFINISHED version 3 scales the function at repeated intervals when the amplitude
% gets too low for the solver as they inevitably decay exponentially
...
%pass this back so this section can be skipped for further calculations
    ...
    op_vec = zeros(numel(set1)*size(nn,1),1);
        ...
        options = odeset('OutputFcn',@outputfun,'AbsTol',1e-12);
