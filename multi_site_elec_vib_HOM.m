function [Time_units,rho_vec,nn,total_prop_op]=multi_site_elec_vib_HOM...
            (Htot,rho_0,N,viblvls,gamma_dru,lambda_dru,Kappa,Kap2,...
            numpoints,tendps,Temp,saveonlyrho00,save_file_name)
        %NOT FINISHED YET
...
%%
 %Htot is the total Hamiltonian for the system of N chromophores and 
 % whatever vibrations are treated quantum mechanically 
 % N is the number of sites / Chromophores
% gamma_dru e.g.=100*ones(N,1); %drude decay constant
% lambda_dru  e.g.= ones(N,1); % weighting of distrubtion
...
vv(:,2:end) = (2*pi/beta)*repmat(1:Kappa,N,1); %These are the Matsuraba frequencies
...
%For reasons I cannot even begin to fathom matlab just overwrites 
%the persistent variables I need every step so this just doesn't work...
    ...
    tt = t(end); rr = rho_v(1:size(saved_rho,2),end);
        ...
        elseif ~strcmp(flag,'done') && ~isempty(flag)
         %   flag
            ...
            %clearvars -except t rho_v flag  %it really didn't like this..
            %clear all the past shit in this function
        ...
        elseif strcmp(flag,'done') %finish integration
           status = 1;
%  if ~isempty(whentosave)
%              non_redundant_points = [true;saved_timee(2:lastpoint)~=0]; 
%             
...
%out  = flag
        if tt >= whentosave(1)
            ...
            non_redundant_points = saved_timee~=0;    
            end
eval([strcat('saved_time',num2str(cnt)) '= saved_timee(non_redundant_points);']);
eval([strcat('saved_rho',num2str(cnt))  '= saved_rho(non_redundant_points,:);']);
