...
for k =1:Kap2
        ...
        uniquetmp = uint64(tmp2 * hashfn(1:k));
        %must be unique as it is the first generated
        uniquelg = logical(uniquetmp);
            for jj = 1:length(cc)
                uniquelg(jj) = ~any(uniquetmp(jj) == uniquetest(1:cn2));
            end
        ...
        scale_fct = sqrt(abs(cc(uniquelg)) + abs(ccI(uniquelg)));
        coup_p1(count1,1+count2:count2+size(tmp2,1)) = ...
            tmp3.' .* scale_fct; 
        coup_m1(1+count2:count2+size(tmp2,1),count1) = ...
            tmp3.' .* cc(uniquelg) ./ scale_fct; 
        coup_m1_acom (1+count2:count2+size(tmp2,1),count1) = ...
            tmp3.' .* ccI(uniquelg) ./ scale_fct;       
