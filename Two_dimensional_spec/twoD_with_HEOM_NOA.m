...
%Don't use the exciton basis, unless you pass the exciton dipole moments
%If static disorder is included then these must be calculated within the
%program.  
if 1==0
...
%generate the N initial density matricies from the first pathways
 ...
 tic
V_ge = V_eg{1}'; NN = numel(V_eg{1});
%initial conditions to propogate
rho_eg = [reshape(V_eg{1},NN,1); zeros((HL-1)*NN,1)]; 
rho_ge = [reshape(V_ge,NN,1); zeros((HL-1)*NN,1)]; 
...
tic 
NN2 = numel(V_fe{4});
V_ge_lio = [reshape(V_eg{4}',1,NN),zeros(1,NN*(HL-1))]; 
V_ef_lio = [reshape(V_fe{4}',1,NN2),zeros(1,NN2*(HL-1))]; 
...
sz_params = [N,HL]; 
V_set = {V_eg{2}',V_fe{2}',V_eg{3},V_fe{3}};
