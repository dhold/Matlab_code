...
if loop_all_pol 
    ...
    sz5 =1; if lg; if islogical(bb) && bb; sz5 =5; end; end;
...
%next those with chiral origins

for j = 1:N

    V1 = V1 + V_n(2:N+1,1,j).*dot(mu(j,:),pol(1,:));
    V2 = V2 + V_n(2:N+1,1,j).*dot(mu(j,:),pol(2,:));
    V3 = V3 + V_n(2:N+1,1,j).*dot(mu(j,:),pol(3,:));
    V4 = V4 + V_n(2:N+1,1,j).*dot(mu(j,:),pol(4,:));

  %  VV1 = VV1 + V_n(N+2:end,2:N+1,j).*dot(mu(j,:),pol(1,:)); %not required
    VV2 = VV2 + V_n(N+2:end,2:N+1,j).*dot(mu(j,:),pol(2,:));
    VV3 = VV3 + V_n(N+2:end,2:N+1,j).*dot(mu(j,:),pol(3,:));
    VV4 = VV4 + V_n(N+2:end,2:N+1,j).*dot(mu(j,:),pol(4,:));
...
% of the isotropic system
    ...
    if lg %also calculate contributions from the magnetic part
        ...
        Vm_eg = V_eg; Vm_fe = V_fe; Vm = zeros(N,1); VVm = zeros(N2,N); 
             ...
             for j=1:N
                Vm = Vm + V_n(2:N+1,1,j).*dot(mm(j,:),bb2(mag_lp,:));
                VVm = VVm + V_n(N+2:end,2:N+1,j).*dot(mm(j,:),bb2(mag_lp,:));
             end
...
if loop_all_pol
    for lp2 = set_cont{lpp}
        if calc_nr
            R1_s(:,:,:,lp2,1+mag_lp) = R1_s(:,:,:,lp2,1+mag_lp)+R1; 
            R4_s(:,:,:,lp2,1+mag_lp) = R4_s(:,:,:,lp2,1+mag_lp)+R4; 
            R6_s(:,:,:,lp2,1+mag_lp) = R6_s(:,:,:,lp2,1+mag_lp)+R6; 
        end
        if calc_rp
            R2_s(:,:,:,lp2,1+mag_lp) = R2_s(:,:,:,lp2,1+mag_lp)+R2; 
            R3_s(:,:,:,lp2,1+mag_lp) = R3_s(:,:,:,lp2,1+mag_lp)+R3; 
            R5_s(:,:,:,lp2,1+mag_lp) = R5_s(:,:,:,lp2,1+mag_lp)+R5; 
        end
%elements 2:5 in R_s 
    end
end
