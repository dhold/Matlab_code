...
CMRmap=interp1(1:9,CMRmap,1:0.5:9);

%fle = matfile('plenio_w_exp_mode_indep_config_more_points2.mat');
%fle = matfile('plenio_cd_config_full_2D_RF_and_mode.mat');
%fle = matfile('2Dcd_with_HEOM_no_mode.mat'); %first one didn't converge
...
data_set = 'pump probe'
%% Rephasing
GSB = imag(fle.R3(:,:,:,cmp))/scale_fct2; 
SE = imag(fle.R2(:,:,:,cmp))/scale_fct2; 
ESA = imag(fle.R5(:,:,:,cmp))/scale_fct2;
data_set = 'rephasing'
fle.chk(ps,1) %check this is actually a valid configuration
%% NonRephasing
GSB = imag(fle.R4(:,:,:,cmp))/scale_fct2; 
SE = imag(fle.R1(:,:,:,cmp))/scale_fct2; 
ESA = imag(fle.R6(:,:,:,cmp))/scale_fct2;
...
contourf(om1_rng/1e4,om3_rng/1e4,squeeze(GSB(:,1,:)-GSB(:,end,:)))
 colormap(CMRmap)
%shading interp
colorbar
%caxis([min1,max1])
...
%% alternative
writerObj = VideoWriter('signal.avi');

open(writerObj);
%Generate initial data and set axes and figure properties.

figure
pcolor(om1_rng/1e4,om3_rng/1e4,squeeze(SE(:,lp,:)+ESA(:,lp,:)));
axis tight
...
legend('show')   
%%
    fig_name = strcat('xxyx_zzz-yyz_GS_with_mode_100fs.png');
set(gcf, 'Color', 'w');
...
%signals are real so will be made of cosines
...
for cmp = size(tmp{1},3) +(1:3)%+(1:size(tmp{2},3))
if sig_type==1   
    
GSB2 = imag(fle.R3(tmp3,pnt,tmp1,cmp))/scale_fct2; 
SE2 = imag(fle.R2(tmp3,pnt,tmp1,cmp))/scale_fct2; 
ESA2 = imag(fle.R5(tmp3,pnt,tmp1,cmp))/scale_fct2;
%{
...
% GSB = real(fle.R3(:,pnt,:,cmp))/scale_fct2; 
% SE = real(fle.R2(:,pnt,:,cmp))/scale_fct2; 
% ESA = real(fle.R5(:,pnt,:,cmp))/scale_fct2; 

% GSB = real(fle.R3(:,pnt,:,cmp))/scale_fct2; 
% SE = real(fle.R2(:,pnt,:,cmp))/scale_fct2; 
% ESA = real(fle.R5(:,pnt,:,cmp))/scale_fct2;
% GSB = GSB-real(fle.R3(:,pnt,:,cmp+3))/scale_fct2; 
% SE = SE-real(fle.R2(:,pnt,:,cmp+3))/scale_fct2; 
% ESA = ESA-real(fle.R5(:,pnt,:,cmp+3))/scale_fct2;
%}
%'rephasing'
...
subplot(3,1,1); 
title(strcat('t=',num2str(t2_range_fs),'fs'))
contourf(om1_rng(om1lg)/1e4,om3_rng(om3lg)/1e4,squeeze(GSB2))
 colormap(CMRmap)
colorbar
caxis([min1,max1])

subplot(3,1,2); 
contourf(om1_rng(om1lg)/1e4,om3_rng(om3lg)/1e4,squeeze(SE2))
 colormap(CMRmap)
colorbar
caxis([min2,max2])
   ...
   colormap(CMRmap)
subplot(3,1,3);  
contourf(om1_rng(om1lg)/1e4,om3_rng(om3lg)/1e4,squeeze(ESA2))
colorbar
caxis([min3,max3])
 colormap(CMRmap)
xlabel('\omega_1  (10^{4} cm^{-1})')
...
tau_list= 1./real(spoles(:));%decay factor
omega_list = imag(spoles(:));
%signals are real so will be made of cosines
%now collect into sin and cosine factors with real prefactors
...
for lp =1:N_order
    ...
    for lp2 = [1:lp-1,lp+1:N_order]
        if abs(omega_list(lp2)+omega_list(lp)) < eps(max(omega_list))
           ...
           cosfct(cnt)= real(a_list(lp)+a_list(lp2)); 
    ...
    elseif abs(omega_list(lp))<= eps(max(omega_list)) %just pure exp decay
        ...
        cosfct(cnt)= real(a_list(lp)); tau_new(cnt) = tau_list(lp);
    end
end
...
%}
 curve = zeros(cnt,length(t_red));
    ...
    %{
%     subplot(2,1,1);
%     plot1 = plot(t_red,real(curve),'Parent',subplot1);
% 
%     xlabel('t_2 (fs)','FontSize',16); ylabel('ES signal, a.u.','FontSize',14);
%     for lp =1:cnt
...
% plot(t_red,[sum(curve);imp_resp],'Parent',subplot2);
    %}
prony_compare_fig(t_red,real(curve(1:4,:)), [sum(curve);imp_resp],om_new*1e3/t_scale)
%%
set(gcf, 'Color', 'w');
