function [prop_op]=H_prop_gen2(H_1,H_2,V1,V2,QQ,const_factor,coup_com,coup_acom,flg)
...
if nargin <9 %normal -i* (H_uu O_uv - O_uv H_vv)
 %L = -1i*(kron(sparse(eye(length(H_2))),sparse(H_1))...
 %           -kron(sparse(H_2).',sparse(eye(length(H_1)))) );        
 L = -1i*(kron(eye(length(H_2)),H_1)-kron(H_2.',eye(length(H_1))));          
        
else %assume operator time dep -i* (O_vu H_uu  - H_vv O_vu )
%  L = -1i*(kron(sparse(H_1).',sparse(eye(length(H_2))))...
 %           -kron(sparse(eye(length(H_1))),sparse(H_2)) );   
...
% reshape(C * B, NN_1*NN_2,1) = kron(B.',eye(NN_1))*reshape(C,NN_1*NN_2,1)  
 %reshape(A * B * A, N^2,1) = kron(A.',A)*reshape(B, N^2,1)
    ...
    Qcom =  kron(eye(size(V_2)),V_1)-kron(V_2.',eye(size(V_1)));
    
    Qacom = kron(eye(size(V_2)),V_1)+kron(V_2.',eye(size(V_1))) ; %anti commutator
    ...
    Qcom = kron(V_1.',eye(size(V_2))) - kron(eye(size(V_1)),V_2); %left acting commutator
    
    Qacom = kron(V_1.',eye(size(V_2))) + kron(eye(size(V_1)),V_2); %anti commutator    
   % Q1 =   Qcom*Qcom
