function [H_el,H_vib,H_e,H_f,e_1,e_2,fock_space_rep,M_p,M_pfull,mu_ex,mag_ex,H_coup] = ...
    generate_ex_vib_ham3(om_dip,om_vib,numvib,displ,mu,mdip,R,pdm) 
...
if ~isempty(om_vib) && isempty(numvib)
        tmp = om_vib; N = length(tmp); clear om_vib displ numvib
    for j = 1:N
        if size(tmp{j},2)==4
    ...
    numvib{j} = tmp{j}(lg,4);
    om_vib{j} =tmp{j}(lg,3) ;
    displ{j} = sqrt(tmp{j}(lg,1)./tmp{j}(lg,3)) ;
...
H_e = om_dip; H_f = zeros(N*(N-1)/2); cnt = 0; 
...
elseif all(cellfun(@isempty,om_vib)) %no vibrations included
    H_vib = 0; H_coup = [];
else
