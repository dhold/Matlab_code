...
%   class(M) is the same as class(perms2(M, flags)).
%   The result of perms2 is acceptable input to another call of perms2.
%   perms2 'unique' is idempotent.  I.e.,
%      t = perms2(M, flag, 'unique') is the same as
%      t = perms2(t, flag, 'unique')
...
%
%   Examples: 
%      perms2(1:2) is              [1 2; 2 1]
%      perms2(1:3, 'even') is      [3 1 2; 1 2 3; 2 3 1]
%      perms2((1:3)/2, 'odd') is   [1.5 1.0 0.5; 0.5 1.5 1.0; 1.0 0.5 1.5]
%      perms2(1:2, 'signs') is     [-1 -2; -1 2; 1 -2; 1 2]
%      perms2([1 0 0],'unique') is [0 0 1; 0 1 0; 1 0 0]
%      perms2([1 1; 2 1]) is       [1 1; 1 2; 2 1]
%      perms2('abc', 'cycles') is  ['abc'; 'cab'; 'bca']
%
...
function M = perms2(M, varargin)        % outer function
  % get input bounds
  if numel(M) == 0, return; end
  if ndims(M) ~= 2,
    error('MATLAB:perms2:input', 'requires mXn input');
  end
  ...
  for arg = 1:nargin-1
    switch lower(varargin{arg})
      ...
      otherwise, error('MATLAB:perms2:badflag', 'unknown flag');
    end
  end
  ...
  if sum(opt(fPerm:fCycle)) > 1
    error('MATLAB:perms2:badflag', 'conflicting flags');
  end
  ...
  function allSigns                    % nesting level 1
    if ~isnumeric(M)
      error('MATLAB:perms2:signs', 'requires numeric argument');
    end
    r = class(M);
    if r(1) == 'u' 
      error('MATLAB:perms2:signedarg', 'requires signed argument');
    end
