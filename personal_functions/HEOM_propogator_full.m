function [ Qtrunc,H_prop_op_L,H_prop_op_R,decay_fct,nn,nn_alt, sec_ops,debug]...
        =HEOM_propogator_full(QQ,cc_com,cc_acom,vv,max_tier,occ_mat,viblvls)
...
coup_com_save2{j} = sparse(1,rng,scale_fct{j},size(nn,1),size(nn,1),tot_couplings)+...
                    sparse(rng,1,cc_com{j}./scale_fct{j},size(nn,1),size(nn,1),tot_couplings);
coup_acom_save2{j} =  sparse(rng,1,cc_acom{j}./scale_fct{j},size(nn,1),size(nn,1),tot_couplings);
...
%%  Calculate the Uber operator that propogates the entire thing
   ...
   %plus_term = total_prop_op;  minus_term = total_prop_op; 
    ...
    H_prop_op = H_prop_op + kron(sparse(coup_acom_save{j}),Uj);
        ...
        %different operators that select particular blocks of the operator
    sec_ops = {tmpg,tmpge,tmpeg,tmpe,tmpgf,tmpfg,tmpef,tmpfe,...
                'tmpg,tmpge,tmpeg,tmpe,tmpgf,tmpfg,tmpef,tmpfe'};

%% calculate decay factor for the higher tiers
temp = kron(const_factor,ones(sz_full^2,1));
decay_fct = - sparse(1:length(temp),1:length(temp),temp);
