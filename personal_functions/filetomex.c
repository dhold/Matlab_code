/* ********************************************************************
   Gateway automatically generated using PHP version of MAKES
   Tool available at http://www.omegacomputer.com/staff/tadonki/automatic_mexfile_matlab.htm
   Depending on the complexity of your code, some minor manual changes might be required */
/* -----------------------------------------------------
MATLAB CALLING COMMAND
  [#include 
#include 
#include 
...
the function name --- needed for linking to Fortran programs on a Sun.]=(ADD_UNDERSCORE);
*/
...
double ADD_UNDERSCORE;
//DOUBLE PRECISION CORRESPONDANCE OF THE OUTPUT
double *gateway_out;

//CONNECT TO THE INPUTS TROUGHT POINTER ASSIGNMENTS
ADD_UNDERSCORE = mxGetScalar(prhs[0]);

//NOW CALL THE MAIN ROUTINE
gateway_out = (double)(ADD_UNDERSCORE);

//NOW ASSIGN THE OUPUT TO THE CORRESPONDING MATLAB VARIABLE
mxSetPr(plhs[0], gateway_out);
mxSetPr(plhs[1], #include 
#include 
#include 
