function [Time_units,rho_vec,nn,total_prop_op]=multi_site_drude_and_brownian_HOM_3...
        ...
        %This is a more general version, allowing for a more general form
        %of the J(omega) for each site and which modes will be included in
        %the truncation operator QQ
...
% Hence H rho - rho H can be expressed with a flattened rho as L *rho
 ...
 LL = -1i*(kron(sparse(eye(length(Htot))),sparse(Htot))...
            -kron(sparse(Htot).',sparse(eye(length(Htot)))) );
...
for j = 1:N
    ...
    Qj = sparse(N,N);
    Qj(j,j) = 1;  Qj = sparse(kron(Qj,eye(viblvls)));
    eyeQ = sparse(eye(length(Qj)));
...
total_prop_op = sparse(length(rho_vec),length(rho_vec));
   %plus_term = total_prop_op;  minus_term = total_prop_op; 
    ...
    Qjsec = sparse(N,N);  Qjsec(j,j) = 1;
    Qjsec = kron(Qjsec,sparse(eye(viblvls)));
    eyeQ = sparse(eye(length(Qjsec))); 
    Qj = -1i*(kron(eyeQ,Qjsec) - kron(Qjsec.',eyeQ)); %commtator
    %keep factor of -1i from -1i tilde(V)^X term
    Uj=  (kron(eyeQ,Qjsec) + kron(Qjsec.',eyeQ)); %anti commutator
...
clearvars -except tend basis_proj convfact rho_vec nn use_reduced_mat ...
            rho_0 Htot numpoints saveuptotier save_file_name non_hermit numwithn
...
if  use_reduced_mat
    ...
    if any(non_hermit)
    [~,~] = ode45(@rhs_dif_eq3,[0,tend],rho_vec,options);
    else
    ...
    if ~isempty(save_file_name)
       outputfun(numpoints,rho_vec(1:(sum(numwithn(1:(saveuptotier+1)))...
           *numel(Htot))),save_file_name);
    else
       outputfun(numpoints,rho_vec(1:(sum(numwithn(1:(saveuptotier+1)))...
           *numel(Htot))),'first_step');        
    end
...
function drho = rhs_dif_eq3(t,rho_vc_red) %Couples to same numwithn value
...
status= 0; pause(0.01) %stops it locking up in windows
