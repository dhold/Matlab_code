function [ Qtrunc_set,H_prop_op_set,nn,nn_alt, sec_ops]=HEOM_propogator_sep...
            (QQ,cc_com,cc_acom,vv,max_tier,occ_mat,viblvls)
...
Qtrunc_set = {Qtrunc_01,Qtrunc_02,Qtrunc_11,Qtrunc_12,...
    'Qtrunc_01,Qtrunc_02,Qtrunc_11,Qtrunc_12'}; %also string saying what ops are
...
for lp = 1:N
    ...
    scale_fct{lp} = sqrt(sqrt(abs(cc).^2+abs(ccI).^2));   %#ok<*AGROW>
    cnt2 = cnt+sz(lp);
...
%%  Calculate the Uber operator that propogates the entire thing
   ...
   %plus_term = total_prop_op;  minus_term = total_prop_op; 
    ...
    tmp = repmat(tmpeg,[size(nn,1),1]);
    H_prop_op_01 =  H_prop_op(tmp ,tmp); %N*viblvls^2
    tmp = repmat(tmpfg,[size(nn,1),1]);
    ...
    H_prop_op_set = {H_prop_op_01,H_prop_op_02,H_prop_op_11,H_prop_op_12,...
                    'H_prop_op_01,H_prop_op_02,H_prop_op_11,H_prop_op_12'};
