%save_file_name =flname;
debug_md = false; vibstates = viblvls;
%use_reduced_mat=0
...
cnt=0; vibst = vibstates;
% for k = 1:length(num_vib)
%     for j =1:num_vib(k)
%         cnt = cnt + 1;
%         vibst(cnt) = viblvls{k}(j);
%     end
% end

for k = 1:length(Time_units)

    temp = TrX(rho00{k},2:1+length(vibst),[N,vibst]);
...
A_0 = sum(cc,2); %in general length N vector
for lp = 1:length(A_0)
coeffmat{lp} = coeffmat_gen(n,n,A0(lp));
end
    
    cc = reshape([cc2R,cc1].',1,numel([cc2R,cc1])); 
    ccI =reshape([cc2I,cc1*0].',1,numel([cc2R,cc1]));  
    %undoall rescaling
    scalefct = repmat(sqrt(abs(cc).^2 + abs(ccI).^2),size(nn,1),1);
