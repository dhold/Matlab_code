function [R_red,R_red_op,Ctilde] = redfield_calc(H_site...
            ,beta,gam_dru,lam_dru,gamma,lambda,om_0,use_markov)
...
vn = 2*pi*(1:20000)/beta;  %take 20000 matsubara, probably more than enough
pm = [+1,-1];
...
%          %real part -> (nom+1)*4*tmp(k)*tmp2(k)*om/(om^2+tmp2(k)^2);
%         tmp3  = tmp3 + sum(4*vn./beta./(vn.^2-tmp(k)^2)./(1i*deltaom-vn));
% 
%         Ctilde(lp,a,b) =Ctilde(lp,a,b) + tmp3*(tmp(k)*tmp2(k));
%         end
%               else
...
%        Ctilde(lp,a,b) = Ctilde(lp,a,b) + tmp4*tmp2(k)*tmp3(k)^2; %all have a lambda*om_0^2 prefac
%                 else
%                 end
%             end
%         end
