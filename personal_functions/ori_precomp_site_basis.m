function  [av_2,av_3,av2_3,av_4,av_5,av2_5]=ori_precomp_site_basis...
                                            (param_set,mu,R,mm);
...
av_2 = zeros(N,N,size(param_set,3));  
if ~isempty(R)
av_5 = zeros(N,N,N,N,size(param_set,3),3); %needs a factor of +i in front
av_3 = zeros(N,N,size(param_set,3));  
else
    av_5 = [];
end
if ~isempty(mm)
av2_5 = zeros(N,N,N,N,size(param_set,3),3); 
av2_3 = zeros(N,N,size(param_set,3));  
else
    av2_5=[];
end
...
if nargout >=4 
pol = param_set(1:4,:,lp) ; %pol = param_set(4:7,:,lp) ;
k_set = param_set(5:7,:,lp); %k_set = param_set(1:3,:,lp);
else
pol = param_set(1:2,:,lp) ;k_set = param_set(3,:,lp);   
%pol = param_set(2:3,:,lp) ;k_set = param_set(1,:,lp);   
end

for k1 = 1:N %slow but only needs doing once
    for k2 = 1:N
        
        %calculate the first order averages
        mu_set = [mu(k1,:);mu(k2,:)];
        ...
        temp1(2,:)= -cross(k_set(1,:),pol(2,:));   temp2(1,:)= -cross(k_set(1,:),pol(1,:));  
         ...
         av2_3(k1,k2,lp) = 1i*(tensor_av(tmp2,temp2)-tensor_av(tmp1,temp1));
              ...
              if nargout >=4 %calculate higher order averages
       for k3 = 1:N 
           for k4 = 1:N  
              lpset =[k1,k2,k3,k4];
        ...
        DeltaR = R(lpset(j),:)-R(lpset(4),:);
         av_5(k1,k2,k3,k4,lp,j) =  1i*(tensor_av([mu_set;DeltaR],[pol;k_j]));
                  end
                  if ~isempty(mm)
         %now on site mag dipole moment        
        ...
        temp1(4,:)= -cross(k_j,pol(4,:));   temp2(j,:)= -cross(k_j,pol(j,:));  
         ...
         av2_5(k1,k2,k3,k4,lp,j) = 1i*(tensor_av(tmp2,temp2)-tensor_av(tmp1,temp1));
                  end
